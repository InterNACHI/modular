...
Version 3.0 introduces a **plugin-based architecture** for module auto-discovery. The monolithic `ModularServiceProvider` has been refactored into a collection of focused, composable `Plugin` classes. This improves extensibility, testability, and separation of concerns.
...
| Before | After |
|--------|-------|
| `config.php` | `config/app-modules.php` |

The published config file location (`config/app-modules.php`) is unchanged.
...
2. Run `composer update internachi/modular`
3. Run `php artisan modules:clear`
