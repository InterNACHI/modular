@php
$modulesDirectory = config('app-modules.modules_directory');
@endphp
---
name: modular
description: Create or modify Laravel modules using `internachi/modular`. Use when the user asks to create a module, add components to a module, scaffold module structure, or work on files in an `{{ $modulesDirectory }}` directory.
argument-hint: <module-name> [component-type]
---

# Laravel Modular Development

You are helping with a Laravel application that uses `internachi/modular` for modular architecture. Modules live in `{{ $modulesDirectory }}/` and follow Laravel package conventions.

## Module Structure

The structure of `{{ $modulesDirectory }}` mimics a standard Laravel application, where what typically would be found in `app` is found in `src`:
...
- Run module tests: `php artisan test {{ $modulesDirectory }}/{module-name}/tests`
